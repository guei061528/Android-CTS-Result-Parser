...
Special_TestCases_Modules = ['VtsHalWifi', 'VtsHalBluetooth'] # "-t testcases" cannot be used for measurements in this module
    ...
    for i in range(len(new_fail_with_no_waive)):
        for j in range(len(old_test_item)):
            if new_fail_with_no_waive[i] == old_test_item[j]:
                ...
                Result.append([new_fail_module_with_no_waive[i], new_fail_with_no_waive[i], 'Failed in Previous Report', 'N/A'])
                try:
...
for i in range(len(Special_TestCases_Modules)):
    for j in range(len(clean_new_fail_module)):
        if Special_TestCases_Modules[i] in clean_new_fail_module[j]:
...
for i in range(len(new_test_incomplete_modules)):
    tmp = 0
    for j in range(len(new_test_module)):
        if new_test_incomplete_modules[i] == new_test_module[j]:
            tmp += 1
    if tmp == 0:
...
if len(New_Fail_Special_Modules) != 0:
...
if len(clean_new_fail_item_final) != 0:
...
with open('Compare_Result_New_Fail.txt', 'w') as out_file:
    for i in range(len(New_Fail_Result)):
        str_line = New_Fail_Result[i][0] + " " + New_Fail_Result[i][1] + "\n"
